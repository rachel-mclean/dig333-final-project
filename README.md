...
## Creator Statement:
For my final project, I created a "secret knock computer lock" — a computer case that opens with a secret knock. When a user executes the secret knock, the case unlocks, and the computer inside becomes removable. When the wrong knock is used, the computer remains locked inside the case. On my device, the knock is applied to an eraser that is attached to the top of the case. The eraser extends the knocking platform of the Piezo knock sensor underneath so that the user has a greater surface area to perform their knock, increasing usability. When the secret knock is applied to this surface, it triggers a mini Servo motor to turn. 

The rigging of the lock consists of a wire loop, a sewing needle, and the Servo motor. The mechanics of the lock are as follows: the Servo motor is secured to the front of the case, just behind a small rectangular cavity. Fastened inside the hinged front of the case is a wire loop. Affixed to the Servo motor is the sewing needle. When the case is locked, the needle is situated inside the wire loop, so that the case is unable to be opened. When the Piezo knock sensor recognizes the secret knock, the Servo motor rotates at an angle of 90 degrees, rotating the sewing needle out of the wire loop, so that the case may be opened and the computer removed. The body of the case is made of wood panels, which were cut using a laser cutter. A silhouette of a door knock is engraved on each of the four sides, in order to subtly suggest the nature of the case. 

The prompt for this project was to make something both useful and useless at once. The secret knock computer lock fulfills both of these conditions. First, note that the computer lock will, in one sense, increase the security of the computer. Unless someone knows the secret knock, they will not easily be able to remove the device from the case. The case is rather bulky, and its lock contraption and other external components appear complicated. This will likely discourage thievery attempts. The novelty of opening a case with a secret knock also adds an element of whimsy to device protection. The Piezo sensor is sensitive enough that knocks can be softly tapped so that they are not easily overheard.

However, if the user of this project forgets their secret knock, they will find themselves in an interesting predicament. It will not be easy to free their computer from the case. Not only does this device rely upon its user’s memory capacity, but using this device requires reliability on both the user’s computer and the Arduino, which presents its own risks. The user will also find themselves in trouble if the Arduino comes to the end of its lifespan while the computer is locked, since the Adafruit Circuit Playground must be plugged into a power source while the computer is locked, in order for the case to work. The user will also find themselves in trouble if their computer dies while inside the case, as the Arduino needs a power source to function. Finally, the biggest risk is a consequence of the main function of the device. If someone were to overhear or oversee the secret knock pattern, the security of the device locked inside becomes compromised. In this way, the secret lock computer lock works against its very purpose.

While the secret knock lock computer case has some interesting utility, it seems as if this contraption creates more problems than it solves. I would recommend that any users of this project or similar projects beware of the risks involved, before relying on the device for security.
